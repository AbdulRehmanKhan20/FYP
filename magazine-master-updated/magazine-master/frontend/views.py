...
def auditorium_reservation(request):
    ...
    if request.method == 'GET':
        ...
        response = requests.post('http://localhost:8000/auditorium/reservation/', data=json.dumps(data),
                                 headers=headers)
...
def auditorium_list_reservation(request):
    list_reservations = AuditoriumReservation.objects.filter(status__exact='Approved')
    if has_admin_group(request.user):
        list_reservations = AuditoriumReservation.objects.filter(status__exact='Approved')
    ...
    try:
        auditorium_listing = paginator.page(page)
    except PageNotAnInteger:
        auditorium_listing = paginator.page(1)
    except EmptyPage:
        auditorium_listing = paginator.page(paginator.num_pages)

    context = {
        'reservations_list': auditorium_listing,
        'page_type': 'List of Approved Reservations:'
    }
...
@login_required
def auditorium_pending_list_reservation(request):
    list_reservations = AuditoriumReservation.objects.filter(status__exact='PENDING', requestor=request.user)
    if has_admin_group(request.user):
        list_reservations = AuditoriumReservation.objects.filter(status__exact='PENDING')
    ...
    context = {
        'reservations_list': auditorium_listing,
        'page_type': 'List of Pending Reservations:'
    }

    return render(request, 'FrontEnd/pending-auditorium-listing.html', context)


@login_required
def auditorium_rejected_list_reservation(request):
    list_reservations = AuditoriumReservation.objects.filter(status__exact='Rejected', requestor=request.user)
    if has_admin_group(request.user):
        list_reservations = AuditoriumReservation.objects.filter(status__exact='Rejected')
    ...
    context = {
        'reservations_list': auditorium_listing,
        'page_type': 'List of Rejected Reservations:'
    }

    return render(request, 'FrontEnd/rejected-auditorium-listing.html', context)
...
def auditorium_update_reservation(request, reservation_id):
    ...
    response = requests.put('http://localhost:8000/auditorium/reservation/' + str(reservation.id) + '/',
...
def auditorium_approve_reservation(request, reservation_id):
    reservation = AuditoriumReservation.objects.get(pk=reservation_id)
    reservation.status = 'Approved'
    reservation.save()

    return redirect('FrontEnd:auditorium_pending_list_reservation')


def auditorium_reject_reservation(request, reservation_id):
    reservation = AuditoriumReservation.objects.get(pk=reservation_id)
...
def daycare_update_reservation(request, reservation_id):
    ...
    response = requests.put('http://localhost:8000/daycare/daycare/' + str(reservation.id) + '/', data=json.dumps(data),
                            headers=headers)
    # pdb.set_trace()
    if response.status_code == 200:
        return redirect('FrontEnd:daycare_list_reservation')
...
def alumni_update(request, reservation_id):
    ...
    response = requests.put('http://localhost:8000/alumni/alumni/' + str(reservation.id) + '/', data=json.dumps(data),
                            headers=headers)
    # pdb.set_trace()
    if response.status_code == 200:
        return redirect('FrontEnd:alumni_list')
...
def settings_detail(request):
    return render(request, 'FrontEnd/settings-detail.html')
